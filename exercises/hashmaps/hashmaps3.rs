...
fn add_to_scores_table(scores: &mut HashMap<String,Team>, team_1_name: String, team_1_score: u8 ,team_2_score: u8) {
    let exited = scores.contains_key(&team_1_name);
    scores.insert(team_1_name.clone(), Team{
        name: team_1_name.clone(),
        goals_scored: team_1_score + if exited { scores[&team_1_name].goals_scored } else { 0 },
        goals_conceded: team_2_score + if exited { scores[&team_1_name].goals_conceded } else { 0 }
    });
    dbg!(&scores);
}
